...
class Calendar extends HTMLElement {
  ...
  slideLeft() {
    this._component.querySelector(".d-calendar").style.marginLeft = `-${
      this.monthWidth * this.navFlag
    }px`;
  }
    ...
    for (let index = 0; index < this.step; index++) {
      ...
      if (!this.months[this.navFlag + this.step]) {
        ...
        if (!this.monthsMap[month.format("YYYYMM")]) {
          ...
          $append(
            monthTagTemplate(month),
            this.querySelector(".d-calendar-row")
          );
        }
      }
      this.slideLeft();
  ...
  getWidth() {
    return this.querySelector(config.monthComponent).getBoundingClientRect()
      .width;
  }
  ...
  bindings() {
    ...
    this.querySelector(".d-calender-navigation-previous").addEventListener(
      "click",
      () => {
        this.movePrevious();
      }
    );
    this.querySelector(".d-calender-navigation-next").addEventListener(
      "click",
      () => {
        this.moveNext();
      }
